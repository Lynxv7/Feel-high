...
export function Artist() {
  return (
    <section id="artist" className="relative overflow-hidden px-6 py-28 sm:px-10 sm:py-36">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        <motion.figure
          ...
          className="group relative mx-auto w-full max-w-[520px] overflow-hidden rounded-xl border border-white/10 bg-white/[0.02]"
        >
          <div className="aspect-[4/5]">
            <img
              src={portrait}
              alt="FEEL HIGH em retrato cinematografico"
          ...
          <figcaption className="absolute bottom-5 left-5 right-5 flex items-center justify-between gap-4 text-overline">
            <span>FEEL HIGH</span>
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            {genres.map((genre, index) => (
              <motion.span
                ...
                className="rounded-full border border-white/12 bg-white/[0.02] px-4 py-2 text-xs uppercase tracking-[0.24em] text-white/55 transition hover:border-[var(--color-ember)]/60 hover:bg-[var(--color-ember)]/10 hover:text-white"
              >
                {genre}
              </motion.span>
            ))}
